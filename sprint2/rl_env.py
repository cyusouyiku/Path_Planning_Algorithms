"""
与 GridEnvironment 对齐的 Gymnasium 环境：局部窗口 + 相对目标向量，供 PPO 等算法训练。
"""
...
from common import octile_heuristic
...
class GridPathfindingGymEnv(gym.Env):
    ...
    def __init__(
        ...
        super().__init__()
        self._m = env or GridEnvironment()
        self._window = int(window)
        if self._window % 2 == 0:
            raise ValueError("window 必须为奇数以便居中")
        ...
        self._cur: tuple[int, int] = self._m.start
        self._steps = 0

    def _patch_and_goal_features(self) -> np.ndarray:
        r, c = self._cur
        gr, gc = self._m.goal
        half = self._window // 2
    ...
    def reset(self, *, seed: int | None = None, options=None):
        ...
        self._steps = 0
        return self._patch_and_goal_features(), {}
    ...
    def step(self, action: int):
        ...
        if self._m.edge_cost(r, c, nr, nc) is None:
            ...
            reward = (
                self._distance_shaping_scale * (old_h - new_h)
                - self._step_penalty
            )
            terminated = False
        ...
        return self._patch_and_goal_features(), reward, terminated, truncated, {}
